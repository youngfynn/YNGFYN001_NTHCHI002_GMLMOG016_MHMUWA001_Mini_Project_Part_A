			  ...
			  if (__HAL_UART_GET_FLAG(&huart2, UART_FLAG_RXNE) == SET)	{
				  ...
				  // if received checkpoint message
					  ...
					  if (check != rcvd_samples)	{
						  sprintf(trans_buffer, "\r\nCheckpoint failed, updating to %d samples received (was %d)", check, rcvd_samples);
						  HAL_UART_Transmit(&huart2, (uint8_t*)trans_buffer, sizeof(trans_buffer), 1000);
...
{
	if (start == 0)
		start = 1;						// set start flag, starts listening for transmissions
	else
		start = 0;						// reset start flag, stops listening for transmissions
...
void invalidSamples(int check)	{
	...
	// update num. of received samples
	rcvd_samples = check;
}
